...
import PagosModal from "./PagosModal.vue";

export default {
  name: "Pagos",
  components: {
    'pagos-detalle-modal': PagosModal
  },
  data() {
    return {
  ...
  methods: {
    ...
    onRowSelected(row) {
    //console.log('Datos del credito ', row[0].id_credito)
   ...
   listado_pagos_clientes(p_credito){
      let v_datos_pagos_cliente = new Pagosmaestro_service();
   // console.log('Recibidoo este credito', p_credito);
      v_datos_pagos_cliente
      .get_pagos_cliente(p_credito)
      .then(response => {
      ...
      this.$bvModal.show("modal-detalle-pagos");
    },
    /*dibujar_totalizador() {
